...
- **Phase 4a (DONE, commit b097bfb):** Rust header readers now accept `(bytes,bytes)` (the critical-path landmine). Added `crate::responses::extract_header_pair()` (str OR bytes); routed all 8 sites through it. Behavior-neutral for the clone; the door is now safe to receive real Starlette responses. parity 151, full 1122, WS 22.
- **Phase 4 — REVISED PLAN for Response:** do NOT bridge clone `Response` to subclass real (its `self.headers`-attr vs real's read-only `headers` property is a structural conflict, and the class gets deleted anyway). Instead the responses work is *delete-and-replace-with-real*, folded into the Phase 5 shim repoint: (1) re-point `responses.rs` 5 cached class pointers + `router.rs` `response_cls` to real `starlette.responses.*`; (2) re-point the shim so `from fastapi.responses import X` is real; (3) delete clone `responses.py`. Gate: parity byte-diff (cookies/headers/charset). This is the most parity-sensitive change — own dedicated pass.
- **Default-on caveat:** flipping `FASTAPI_TURBO_ADAPTER` default-on is validated (full-suite-ON 1122) BUT would expose the flaky cross-file streaming-early-exit test in normal CI — stabilize that (or let the responses re-point resolve it) before flipping.

## Phase 4b/5 (responses + shim repoint) — analysis, NOT yet executed
Re-read the workflow's http-types + compat-shim maps. Key constraints confirmed:
- **Hard prerequisite chain** (from the maps): type-bridge → adapter **default-on** → shim flip (Phase 1: stop overwriting sys.modules, monkey-patch only `fastapi.FastAPI`→accelerated) → door retarget (`router.rs` `request_cls`/`response_cls` → real starlette) → delete clone `requests.py`/`responses.py`. The shim re-point is the "master gate"; the door binds clone Request/Response **by module path** (independent of the shim), so it needs its own retarget.
- **responses.py blocker:** `apply_injected_response` (router.rs) reads `obj.headers` as a **PyDict** — real `Response.headers` is a `MutableHeaders` (cast fails there); must fix to read `.items()`/`.raw` before re-pointing. (raw_headers bytes already handled in 4a.) Plus set_cookie/charset byte-format must be verified for the parity byte-diff.
- **requests.py blocker:** the door pre-stashes the body in `scope['_body']` with NO receive callable; real `Request.body()` reads `receive()`. Either keep a thin door Request subclass (current) or add a receive() to the scope before deleting.
- **datastructures.py NOT a clean drop-in (verified):** clone call sites use `URL(self._scope)` / `Headers(raw)` POSITIONALLY (requests.py:75, websockets.py:153/163/146), but real starlette is `URL(url='', scope=None)` / `Headers(headers=None, raw=...)` — re-export would silently pass scope as the url string. Needs per-call-site fixes (keyword args) first.

**Conclusion:** every remaining deletion — even the "low-risk" ones — has hidden per-call-site coupling, and the responses/shim repoint is gated on adapter-default-on (itself gated on streaming-test stability). These are careful dedicated passes, not a single sweep. Recommended order: stabilize streaming flake → adapter default-on → datastructures call-site fixes + re-export → requests/responses door-retarget + shim flip → delete clone modules.

## Phase 4 / default-on (DONE, commits f3d8375 + e943f48)
- Stabilized the flaky streaming early-exit test (poll for cancellation vs fixed sleep).
- Flipped the adapter **DEFAULT-ON** (`_adapter_route_info`: opt-OUT via `FASTAPI_TURBO_ADAPTER=0`). No-flag is now the real-FastAPI introspection path (~407 routes); clone `_introspect` is the fallback for declined routes + reachable via `=0`. Validated: no-flag parity 151 / full 1122 / WS 22; opt-out parity 151.

## Shim-flip caveat (next gate)
Default-on is the prerequisite the plan cited, BUT the shim flip (`from fastapi import X` → real) still has a live risk: routes the adapter DECLINES (un-bridged TYPES `Response`/`UploadFile`/`WebSocket`, Form/File, async-gen deps) fall to the CLONE `_introspect` path, whose special-param detection uses `isinstance(annotation, clone-type)`. After the flip those handlers carry REAL types → clone detection misses them. So before/with the shim flip, either (a) make clone `_introspect`'s type detection accept real starlette types (clone Request/BG already subclass real; Response/UploadFile/WebSocket need real-type isinstance), or (b) bridge those types so the adapter engages them (no clone fallback). Cheapest: (a) — point `_introspect`'s special-injection isinstance checks at real starlette types. Do that FIRST, then the shim flip is safe.

## Path B (reduce adapter declines → delete _introspect)
- **B.0 (done, commit prior):** `_introspect` special-param detection recognizes REAL starlette types (dual clone+real) — keeps the declined-route clone fallback correct through the shim flip.
- **B.1 Form routes (done, commit 7e743bd):** dropped the Form/File declines; Form (and `bytes=File()`) routes now engage. Required completing the shim's `fastapi.types` (real FastAPI's `_should_embed_body_fields` lazy-imports `UnionType` from it). Engagement 407→420. parity 151, full 1122.
- **B.remaining (to drive declines that USE `_introspect` → ~0, then delete it):** UploadFile-annotated routes (needs UploadFile bridged to real starlette UploadFile), `response: Response` routes (Response delete-and-replace), route/router/global deps (combined deps are real Depends now — likely engageable, but security-sensitive: validate hard), APIKey security schemes, async-gen deps. NOTE dynamic `/docs`+`/openapi.json` and WebSocket routes decline but do NOT use `_introspect.introspect_endpoint`, so they don't block deleting it. Once the app.run clone-introspection path is unused → delete `_introspect.py` → then routing/OpenAPI→real → shim flip → delete the dispatcher.
- **B.2 route/router/global deps (done, commit f3581e9):** dropped the combined-deps decline; route-level deps (name=None) emit as non-handler-params and RUN (verified 401-enforcement). Engagement 420→429.
- **B finding — bridging path largely exhausted:** UploadFile + Response BOTH have read-only-property conflicts (content_type / headers) with real starlette, so they CANNOT be bridged (subclass) like Request/BG/markers — they need **delete-and-replace-with-real**. The remaining declines that use `_introspect` (UploadFile, Response, async-gen deps, APIKey schemes) therefore need the coordinated shim-flip / door-retarget / delete-and-replace, not more one-off bridges.

## Inflection point
Incremental engagement is largely banked: markers, types (Request/HTTPConnection/BackgroundTasks), OAuth2/HTTP security, Form, route/global deps → **429 routes** on the adapter. The remaining clone deletion is the BIG coordinated set, and two viable endgames:
