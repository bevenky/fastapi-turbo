...
* **Sandboxed env** (`socket.bind('127.0.0.1', 0)` raises `PermissionError` at the kernel level — distinct from the conftest's `LOOPBACK_DENIED` flag): the canonical FastAPI 0.136.0 gate currently produces ~112 failures spread across mid-sized buckets (yield-dep websocket scopes ~15, body-tutorial JSON-shape edges, websocket router/tutorial002, sub-app validation contexts, dependency-duplicates combined-body for handler+dep, json_type Pydantic Json[T] form/query). R45 added async-generator handler auto-streaming (NDJSON default + ``response_class`` override for SSE / custom streamers) and yield-dep swallow detection raising ``FastAPIError`` per FA's "raising an exception and a dependency with yield without raising again" contract. Net: 818 → 112 (-706 failures across R39 closure through R45).
