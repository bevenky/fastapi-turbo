...
* **Sandboxed env** (`socket.bind('127.0.0.1', 0)` raises `PermissionError` at the kernel level — distinct from the conftest's `LOOPBACK_DENIED` flag): the canonical FastAPI 0.136.0 gate currently produces ~243 failures, concentrated in yield-dep `gen.throw()` propagation (~40) and miscellaneous tutorial body/dependency edges. R39 + R40 + R41 closed the bulk of the in-process dispatcher drift the audit flagged: TestClient lifespan-failure swallow, raw-ASGI MW chain shutdown swallow, pydantic ``ctx`` stripping, ``/docs/oauth2-redirect`` registration, ``_ASGISyncClientShim.base_url`` accessor, ``response: Response`` shared-instance contract between deps and handlers, ``Optional[list[X]] = None`` default preservation in query/header/form, multi-value urlencoded form parsing, form/file ``loc`` prefix (``body`` not ``form``), parameter-model builder (``pm_<var>__<field>`` extraction → builder-dep wiring), async ``__call__`` on class-instance deps awaited, ``Depends(SomeClass)`` form/file params on ``__init__`` extracted (OAuth2PasswordRequestForm), and form parser fires for nested-dep form params. Net: 818 → 243 (-575 failures across R39 closure + R40 + R41 batches).
