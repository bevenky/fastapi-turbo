...
* **Sandboxed env** (`socket.bind('127.0.0.1', 0)` raises `PermissionError` at the kernel level — distinct from the conftest's `LOOPBACK_DENIED` flag): the canonical FastAPI 0.136.0 gate currently produces ~136 failures spread across mid-sized buckets (yield-dep websocket scopes ~12, body-tutorial JSON-shape edges, dataclass response_model edges, websocket router/tutorial002, sub-app validation contexts, body-alias missing emit). R39 + R40 + R41 + R42 + R43 closed the bulk of the in-process dispatcher drift; the most recent batch added multi-extra-dep error accumulation, ``Header(validation_alias=...)`` extraction, and bare ``list`` / ``set`` annotations treated as sequence params. Net: 818 → 136 (-682 failures across R39 closure through R43).
