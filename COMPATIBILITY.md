...
A per-feature map of where fastapi_turbo sits against its stated compat target (FastAPI 0.138.1 + Starlette 1.3.1). `Full` means the feature is observably indistinguishable from upstream in user code. `Partial` means the surface exists but some sub-behaviour diverges. `Different-by-design` flags intentional deviations that aren't parity bugs.

Status: **all enforced upstream FastAPI tests pass** under the `import fastapi_turbo` sys.modules shim via the canonical gate (`scripts/run_external_compat_gates.sh fastapi`) — zero failed, zero skipped, zero xfailed. The gate deselects upstream-FastAPI tests that aren't ours to fix (`tests/benchmarks/test_general_performance.py` opt-in via `--codspeed`, `tests/test_pydantic_v1_error.py` skipped by upstream on py3.14+, and 4 cases in `tests/test_tutorial/test_query_params_str_validations/test_tutorial006c.py` xfailed by upstream per [fastapi/fastapi#12419](https://github.com/fastapi/fastapi/issues/12419) — upstream's own decisions, not compat regressions). **As of 2026-07-15 the gate excludes NO tracked product gaps** — the three former exclusions are all enforced: `tests/test_frontend.py` (60/60 — `app.frontend()` served at the door's 404 fallback), `tests/test_router_include_context.py` (38/38 — real `_IncludedRouter` lazy semantics, per-request `scope["fastapi"]["effective_route_context"]`, `iter_route_contexts` uniqueness), and the sentry profiler pair `test_active_thread_id[/sync|/async/thread_ids]` (APM patch-detection→delegation: an external monkey-patch of `fastapi.routing.get_request_handler` is detected by identity at route-build and those routes delegate to the real handler chain — generalizes to Datadog/New Relic-style APM patching). `test_traceback_for_dependency_with_yield` is ENFORCED on all runtimes since 2026-07-15 — the door now attaches the original `__traceback__` to exception objects crossing the PyO3 boundary on Python <3.12 (`src/responses.rs::err_value_with_tb`; on 3.10/3.11 PyO3's legacy `PyErr_Fetch` triple kept the traceback in a separate slot, so `PyErr::value()` handed Python an exception with `__traceback__ = None` and the TestClient re-raise fabricated frames rooted in turbo instead of the user's dependency; pinned by `tests/stress/test_traceback_provenance.py`). The gate script also bails out with a `maturin develop` instruction when the loaded `_fastapi_turbo_core.*.so` is older than any `src/*.rs` source — fixes the recurring auditor reports of 888 failures (correlated with stale pre-R34 builds). Threshold regression at `tests/stress/test_r36_regressions.py::test_upstream_fastapi_gate_passes_canonical_threshold` calls the canonical script and fails loudly below 3000 pass / non-zero failed/skipped/xfailed (returncode-first). Sentry integration surface (sentry-sdk 2.42.0, ASGI + FastAPI trees): 75/75 under the shim, no deselects — the historical 33/33 + 89/89 split predates the 2.42.0 surface reduction.

**Latest verified test counts** (auto-generated by CI is preferred, hand-pinned here when CI hasn't run yet):

| Suite | Result | Snapshot date | Snapshot HEAD |
|---|---|---|---|
| Upstream FastAPI canonical script (`run_external_compat_gates.sh fastapi`, tag 0.138.1; deselects only upstream's own opt-outs — NO tracked-gap ignores remain) | 3,243 passed, 10 deselected | 2026-07-15 | post wave-2 (frontend + include-context + sentry delegation + traceback provenance all enforced) |
| Own suite (`pytest tests/`, `FASTAPI_TURBO_SKIP_SUBPROCESS_DRIFT=1`) | 1,090 passed, 3 skipped | 2026-07-15 | post wave-2 (pool-1 prune, APM/traceback/minimal-install regressions added) |
| Sentry SDK FastAPI + ASGI integration | 75 passed, no deselects (test_active_thread_id pair enforced via APM patch-detection→delegation) | 2026-07-15 | post wave-2 (sentry-sdk 2.42.0) |
...
* **Sandbox / restricted CI** (`socket.bind('127.0.0.1', 0)` denied with `PermissionError` in the pytest process): `tests/conftest.py` detects this at session start via a one-shot bind probe and sets `LOOPBACK_DENIED = True`. Tests that exercise the in-process / ASGI dispatch path run cleanly via a sandbox-aware `server_app` fixture (exec's the app in-process, routes `httpx.*` through `ASGITransport`); tests that genuinely need a real loopback port are skipped via `@pytest.mark.requires_loopback`.
  ...
  - **Counts at the R53 watermark**, measured on macOS Apple Silicon
    ...
    1. *True sandbox + FORCE env var* — every bind raises AND
       `FASTAPI_TURBO_FORCE_LOOPBACK_DENIED=1`: 859 pass, 234 skipped.
    ...
    2. *Forced-fail bind only* — monkey-patched `socket.socket.bind`
       to raise `PermissionError`, no env var: 859 pass, 234 skipped
    ...
    3. *Bind works, no env var* (normal dev box): 1093 pass, 0 skipped
       — full coverage. This is the "happy path" that CI / release
       runners hit. Drift detector at `tests/stress/test_r37_regressions.py`
       allows ±2 vs the doc claim (one nested-pytest drift detector can
       flake by 1 under the parent run — run the r3{3,6,7} detectors in
       isolation, as prescribed above).
...
>
> 1. **157 parity-runner tests** (`tests/parity/`) — drive both upstream FastAPI AND turbo as subprocess servers on real loopback ports and diff the responses. The whole compatibility claim against FastAPI 0.138.1 hangs off these.
> 2. **24 `test_websocket.py`** — drive `websockets.sync.client.connect` against a real subprocess WS server. Catches real-WS-protocol regressions (handshake, ping/pong, close codes over the wire) that the in-process WS path can't reproduce.
...
| `app.mount("/sub", sub_app)` for FastAPI / StaticFiles / ASGI | Full | Sub-app `HTTPException`s (e.g. real StaticFiles' missing-file 404) render through the parent app's exception handlers on every path — matching real Starlette's `ExceptionMiddleware`-wraps-mounts contract. |
| `app.frontend()` / `router.frontend()` (FastAPI 0.138 SPA serving) | Full | Served at the door's 404 fallback via real `Router._match_low_priority` + `_FrontendStaticFiles`: fallback modes (`auto`/`index.html`/`404.html`/`None`), Accept-header navigation detection, longest-prefix multi-frontend, traversal/symlink protection, OpenAPI exclusion. Upstream `tests/test_frontend.py` 60/60 under the shim; enforced in the CI gate. |
| 0.138 live include-context (`_IncludedRouter` lazy semantics) | Full | `include_router` appends exactly one real `_IncludedRouter` (upstream-shaped `app.routes`); live route addition after inclusion, repeated-inclusion contexts (disambiguated per request via `scope["fastapi"]["effective_route_context"]`), indirect-cycle rejection, custom `matches`/`handle` dispatch hooks, and `iter_route_contexts` uniqueness all match upstream. `tests/test_router_include_context.py` 38/38 ENFORCED in the gate. |
...
| `SentryAsgiMiddleware(app)` legacy wrap | Full | APM patch-detection→delegation (2026-07-15): originals of `fastapi.routing.get_request_handler` are captured at import; when an APM SDK monkey-patches it, affected routes delegate to the real handler chain so the wrapper (profiler thread-tracking, naming, extractor) actually runs — with `scope["endpoint"]`/`["route"]` populated and turbo's inline sentry helpers standing down (one transaction per request, no double envelopes across the door's thread hop). Full sentry surface 75/75, no deselects; regression-pinned in `tests/test_apm_patch_delegation.py`. Generalizes to any `get_request_handler`-patching APM. |
...
| `fastapi_turbo.http.Client` | Partial | Httpx-compatible request-construction surface, Rust-reqwest-backed. Probe-confirmed parity with `httpx.Client.build_request` for: base_url joining (relative / leading-slash / `..` / `.` / absolute), `data` only, `files` only, `data + files` merged into one multipart, `json`, raw `content`, query-param replace-existing-query semantics, cookies header. See `tests/test_http_client_httpx_parity.py`. Surfaces NOT yet under contract test: redirect chain, auth flow generator, event hooks, HTTP/2 negotiation, proxies, streaming response — call out a gap and add a parity case before relying on it. |
...
| Shim `import fastapi` / `import starlette` | Full | Installed on `import fastapi_turbo`. `fastapi.__version__` reports the REAL installed fastapi's version (`0.138.1` — the real package is underneath since the pip-fastapi pivot), not our own `0.1.0`. |
...
./scripts/run_external_compat_gates.sh fastapi   # upstream FastAPI 0.138.1 only
