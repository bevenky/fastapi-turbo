...
* **Sandboxed env** (`socket.bind('127.0.0.1', 0)` raises `PermissionError` at the kernel level — distinct from the conftest's `LOOPBACK_DENIED` flag): the canonical FastAPI 0.136.0 gate currently produces ~119 failures spread across mid-sized buckets (yield-dep websocket scopes ~12, body-tutorial JSON-shape edges, async-generator handler streaming, websocket router/tutorial002, sub-app validation contexts, dependency-duplicates combined-body for handler+dep). R39 + R40 + R41 + R42 + R43 + R44 closed the bulk of the in-process dispatcher drift; R44 added Body alias / validation_alias remap in 422 ``loc``, top-level missing ``input=None`` (nested keeps the Pydantic parent), and the simple non-embed body single-param missing now emits ``["body"]`` per FA's contract. Net: 818 → 119 (-699 failures across R39 closure through R44).
