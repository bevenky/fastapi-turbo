...
* **Sandboxed env** (`socket.bind('127.0.0.1', 0)` raises `PermissionError` at the kernel level — distinct from the conftest's `LOOPBACK_DENIED` flag): the canonical FastAPI 0.136.0 gate currently produces ~155 failures spread across mid-sized buckets (yield-dep websocket scopes ~12, body-tutorial JSON-shape edges, dataclass response_model edges, websocket router/tutorial002, sub-app validation contexts). R39 + R40 + R41 + R42 closed the bulk of the in-process dispatcher drift: TestClient lifespan-failure swallow, raw-ASGI MW chain shutdown swallow, pydantic ``ctx`` stripping, ``/docs/oauth2-redirect`` registration, ``_ASGISyncClientShim.base_url`` accessor, ``response: Response`` shared-instance contract, ``Optional[list[X]] = None`` default preservation, multi-value urlencoded form parsing, form/file ``body`` loc prefix, parameter-model builder, async ``__call__`` on class-instance deps awaited, ``Depends(SomeClass)`` form/file params extracted (OAuth2PasswordRequestForm), form parser fires for nested-dep form params, yield-dep ``gen.throw()`` propagation in error path with BG-before-teardown ordering, ``functools.wraps`` wrapped generator dep detected, multi-error form 422 accumulation, ``list[bytes]`` file param reads upload contents, JSON-decode error ``ctx`` shape, body-embed missing-field ``loc``, and ``RequestValidationError.endpoint_ctx`` augmentation. Net: 818 → 155 (-663 failures across R39 closure + R40 + R41 + R42 batches).
