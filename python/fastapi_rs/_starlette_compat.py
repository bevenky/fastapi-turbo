"""Starlette-only compatibility classes that fastapi-rs doesn't otherwise
...
# Re-export fastapi-rs's APIRoute/APIRouter as starlette.routing.Route/etc.
...
class Host:
    ...
    fastapi-rs doesn't route by host natively; this is a stub for
...
class HTTPEndpoint:
    ...
    async def method_not_allowed(self, request):
        from fastapi_rs.responses import PlainTextResponse
...
class WebSocketEndpoint:
    ...
    async def _dispatch(self):
        ...
        # methods can use the standard fastapi-rs API.
...
async def _send_response(response, send) -> None:
    """Serialise a fastapi-rs Response object over ASGI send."""
    if response is None:
        return
...
class FormParser:
    ...
    Only kept for import-compat; fastapi-rs parses forms in Rust.
    """
...
class SchemaGenerator:
    ...
    convention) and merges them into an OpenAPI base. fastapi-rs's native
