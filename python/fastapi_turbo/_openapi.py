...
def generate_openapi_schema(
    *,
    ...
    if routes:
        try:
            from fastapi_turbo.routing import APIRoute as _APIRoute
            from fastapi_turbo._introspect import introspect_endpoint as _intro
    ...
    for route in routes:
        ...
        try:
            from fastapi_turbo.responses import EventSourceResponse as _ESR_reg
        ...
        for sub in _flatten_annotation_types(route.get("response_model")):
            if _is_sse_for_reg:
                try:
                    from fastapi_turbo.sse import ServerSentEvent as _SSE_REG
...
def _build_operation(route: dict[str, Any], method: str) -> dict[str, Any]:
    ...
    if _suppress_content:
        ...
        try:
            from fastapi_turbo.sse import ServerSentEvent as _SSE_CLS
...
def _build_parameter(param: dict[str, Any]) -> dict[str, Any]:
    ...
    if param.get("_param_model_field_info") is not None:
        from fastapi_turbo._introspect import _PARAM_MODEL_MISSING as _PMM
...
def _collect_schemas(
    ...
    _is_sse_route = False
    try:
        from fastapi_turbo.responses import EventSourceResponse as _ESR_cs
    ...
    for sub in _flatten_annotation_types(resp_m):
        if _is_sse_route:
            try:
                from fastapi_turbo.sse import ServerSentEvent as _SSE_CS
...
def _build_callbacks(
    ...
    def _route_entry(cb_route, prefix: str = "") -> tuple[str, str, dict[str, Any]]:
        ...
        try:
            from fastapi_turbo._introspect import introspect_endpoint as _ie
